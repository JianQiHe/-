...
@interface FirstProductdetail()<UIScrollViewDelegate>

@end

@implementation FirstProductdetail
{
...
-(void)viewDidLoad
{
    ...
    scrollview = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, Width, 250)];
    scrollview.delegate = self;
    scrollview.showsHorizontalScrollIndicator = NO;
...
#pragma mark -----------------优惠购买及商品资讯-----------------------
    ...
    buyBtn.tag = 1001;
    [buyBtn addTarget:self action:@selector(AddShopCar:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)Productlist
{
    NSString *str=[NSString stringWithFormat:@"http://jiazhuang.siruoit.com/index.php?api/api-product/%@-1",self.Cat_Str];
...
-(void)ProductDetail
{
    NSString *str=[NSString stringWithFormat:@"http://jiazhuang.siruoit.com/index.php?api/api-detail/%@",dict2[@"data"][i][@"product_id"]];
...
{
    [self createCommodityView:button];
//    [MBProgressHUD showError:@"未登录!"];
    
}

//商品详情
- (void)createCommodityView:(UIButton *)button{
   ...
   contentDic = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                          imgStr, @"imgName",
                          dict[@"data"][0][@"price"], @"price",
                         dict[@"data"][0][@"name"], @"title",
                 dict[@"data"][0][@"shop"][0][@"name"],@"shopName",
...
- (void)confirmBtnAction:(UIButton *)sender{
    ...
    switch (sender.tag) {
        case 1000:
            ...
            break;
        case 1001:
            [self popSureOrderControllerView];
            break;
...
- (void)popSureOrderControllerView{
    SureOrderViewController *controller = [[SureOrderViewController alloc] initWithNibName:@"SureOrderViewController" bundle:nil];
    controller.navigationItem.title = @"确认订单";
    [contentDic setObject:_numberLab.text forKey:@"number"];
    controller.contDic = contentDic;
